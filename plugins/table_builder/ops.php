...
function build($argv)
{
  ...
  if(!$error)
  {
    ...
    {
      if($data['relation']['type'] != 'one-to-many' && $data['relation']['type'] != 'many-to-many')
      {
        $query .= $data['field'].' '.strtoupper($data['type']);
        ...
        {
          $query .= 'INDEX '.$name.'_rel('.$data['relation']['foreign']['table'].'_id)';
          $query .= ','."\n".'FOREIGN KEY('.$name.'_id) REFERENCES '.$data['relation']['foreign']['table'].'('.$data['relation']['foreign']['field'].') ON DELETE CASCADE';
          $query .= ",\n";
        }
      }
    }
